...
def read_files(process,variation,variable):
    truth_int = 0 if process == 'qcd' else 1
    vartype_int = -99
    if variation == 'nominal':
        vartype_int = -1
    elif variation == 'fsrRenHi':
        vartype_int = 1
    elif variation == 'fsrRenLo':
        vartype_int = 2
    elif variation == 'herwig':
        vartype_int = 3
    arr = None
    #print("/mnt/T2_US_MIT/hadoop/cms/store/user/bmaier/cl/outfiles/july29_%s/%s_*%s*h5"%(whichtraining,process,variation))
    counter = 0
    for i in tqdm.tqdm(glob.glob("../plots/aug16_t0p1_halfData/*h5")):
    #for i in tqdm.tqdm(glob.glob("/mnt/T2_US_MIT/hadoop/cms/store/user/bmaier/cl/outfiles/july29_t0p1_nloss/%s_*%s*h5"%(process,variation))):
        counter += 1
        ...
        with h5py.File(i,'r') as f:
                ...
                truth_mask = (np.array(f['truth_label'][()]).flatten() == truth_int)
                vartype_mask = (np.array(f['vartype'][()]).flatten() == vartype_int)
                #print(truth_mask,vartype_mask,np.array(f['truth_label'][()]),np.array(f['vartype'][()]))
                feature_arr = feature_arr[truth_mask & vartype_mask]
                #feature_arr = feature_arr[:,:8]
                #feature_arr = normalize(feature_arr, axis=1)
...
ax_ratio4.set_xlabel("NN feature %i"%whichfeat)
...
plt.savefig("/home/tier3/jkrupa/public_html/cl/%s_feat_%s.png"%(whichtraining,str(whichfeat)),dpi=300,bbox_inches='tight')
plt.savefig("/home/tier3/jkrupa/public_html/cl/%s_feat_%s.pdf"%(whichtraining,str(whichfeat)),dpi=300,bbox_inches='tight')
