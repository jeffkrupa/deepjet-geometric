...
from deepjet_geometric.datasets import CLV1#, CLV1ROOT
...
data_test = CLV1(args.ipath,ratio=True)
...
def test():
    ...
    for data in tqdm.tqdm(test_loader):
        ...
        with torch.no_grad():
            ...
            for iB in range(BATCHSIZE):
                if len(out[0]) == iB: break
                #loss = contrastive_loss(out[0][0::2],out[0][1::2],0.2)
                #print("OUTTTT")
                #print("latent features",out[0][iB].numpy().shape)
                #print(out[iB][0].numpy())
                #print(data.x_jet.shape)
                #print("jet features", data.x_jet[N_JET_FEATURES*iB:N_JET_FEATURES*(iB+1)].numpy().shape)
                #print(np.concatenate((out[0][0].numpy(),data.x_jet.numpy()),axis=None))
                #print(data.y[N_JET_FEATURES*iB:N_JET_FEATURES*(iB+1)])
                latent_variables = out[0][iB]
                latent_variables = F.normalize(latent_variables,dim=0)
                #print(latent_variables)
                #jet_features.append(np.concatenate((out[0][iB].cpu().numpy(),data.x_jet[N_JET_FEATURES*iB:N_JET_FEATURES*(iB+1)].cpu().numpy()),axis=None))
                jet_features.append(np.concatenate((latent_variables.cpu().numpy(),data.x_jet[N_JET_FEATURES*iB:N_JET_FEATURES*(iB+1)].cpu().numpy()),axis=None))
                total_loss += 1
                vartype    .append(data.x_vartype[iB:iB+1].cpu().numpy())
                truth_label.append(data.y        [iB:iB+1].cpu().numpy())
    ...
    ofile = h5py.File(args.opath+"/output.h5",'w')
    #print("XXX vartype",vartype)
    #print("ZZZ truthlabel",truth_label)
    ofile.create_dataset('jet_features', data=jet_features)
    ofile.create_dataset('truth_label', data=truth_label)
    ofile.create_dataset('vartype', data=vartype)
    ofile.close()
